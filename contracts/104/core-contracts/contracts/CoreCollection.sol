  ...
  ERC721Payable
{
  ...
  function initializeClaims(
    bytes32 _root
  ) external onlyOwner onlyNotClaimableSet onlyValidRoot(_root) {
   ...
   */
  function setRoyaltyVault(
    address _royaltyVault
  ) external onlyVaultUninitialized {
    require(
  ...
  function setStartingIndex() public {
    ...
    startingIndex =
      (uint256(keccak256(abi.encodePacked('CoreCollection', block.number))) %
        maxSupply) +
      1;
   ...
   */
  function batchMint(address _to, uint256 _amount, bool _isClaim) private {
