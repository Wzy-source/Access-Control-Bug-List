...
contract TroveManager is TroveManagerBase, ITroveManager, ReentrancyGuard {
  ...
  function getCurrentICR(
    ...
    (newColls memory colls, uint currentYUSDDebt) = _getCurrentTroveState(
      _borrower
    );
  ...
  function _getCurrentTroveState(
    address _borrower
  ) internal view returns (newColls memory colls, uint YUSDdebt) {
  ...
  function _getPendingCollRewards(
    address _borrower
  ) internal view returns (newColls memory pendingCollRewards) {
  ...
  function getPendingYUSDDebtReward(
    address _borrower
  ) public view override returns (uint pendingYUSDDebtReward) {
    if (Troves[_borrower].status != Status.active) {
      return 0;
    }
    address[] memory allColls = whitelist.getValidCollateral();
    ...
    for (uint256 i; i < allCollsLen; ++i) {
      ...
      uint assetYUSDDebtReward = stake.mul(rewardPerUnitStaked).div(
        DECIMAL_PRECISION
      );
  ...
  function redistributeDebtAndColl(
    ...
    for (uint256 i; i < tokensLen; ++i) {
      ...
      uint proratedDebtForCollateral = collateralVC.mul(_debt).div(
        totalCollateralVC
      );
      uint dec = IERC20(token).decimals();
      uint CollNumerator = amount.mul(10 ** dec).add(
        lastCollError_Redistribution[token]
      );
      uint YUSDDebtNumerator = proratedDebtForCollateral
      ...
      if (totalStakes[token] != 0) {
        ...
        uint YUSDDebtRewardPerUnitStaked = YUSDDebtNumerator.div(
  ...
  function _calcRedemptionFee(
    ...
    uint redemptionFee = _redemptionRate.mul(_YUSDRedeemed).div(
      DECIMAL_PRECISION
    );
  ...
  function getCurrentTroveState(
    address _borrower
  ) external view override returns (address[] memory, uint[] memory, uint) {
    (newColls memory colls, uint currentYUSDDebt) = _getCurrentTroveState(
      _borrower
    );
