...
contract TroveManagerRedemptions is TroveManagerBase, ITroveManagerRedemptions {
  ...
  function redeemCollateral(
    ...
    if (
      ...
      while (
        ...
        troveManager.getCurrentICR(currentBorrower) < MCR
      ) {
  ...
  function redeemCollateralSingle(
    ...
    require(
      troveManager.getCurrentICR(hints.target) >= MCR,
      "TMR:Trove is underwater"
    );
...

    {
      ...
      uint tokenAmountToRedeem = singleRedemption
        .YUSDLot
        .mul(colls.amounts[i])
        .div(singleCollUSD);
    ...
    if (troveDebt == YUSD_GAS_COMPENSATION) {
      ...
      uint256 newICR = LiquityMath._computeCR(
        _getVC(colls.tokens, colls.amounts),
        troveDebt
      );
  ...
  function _redeemCollateralFromTrove(
    ...
    // Decrease the debt and collateral of the current Trove according to the YUSD lot and corresponding Collateral to send
    uint256 newDebt = (troveManager.getTroveDebt(_borrower)).sub(
      singleRedemption.YUSDLot
    );
    ...
    if (newDebt == YUSD_GAS_COMPENSATION) {
      ...
      if (
        newICR >= _partialRedemptionHintICR.add(2e16) ||
