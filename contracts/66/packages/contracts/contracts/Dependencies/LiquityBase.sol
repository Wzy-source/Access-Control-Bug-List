...
contract LiquityBase is ILiquityBase, YetiCustomBase {
  ...
  uint public constant YUSD_GAS_COMPENSATION = 200e18;
  ...
  ) internal view returns (uint totalVC) {
  ...
  function _getUSDColls(
    newColls memory _colls
  ) internal view returns (uint USDValue) {
    uint256 tokensLen = _colls.tokens.length;
    for (uint256 i; i < tokensLen; ++i) {
      uint valueUSD = whitelist.getValueUSD(
        _colls.tokens[i],
        _colls.amounts[i]
      );
