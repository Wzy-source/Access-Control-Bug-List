...
contract YETIStaking is IYETIStaking, Ownable, CheckContract, BaseMath {
  ...
  function increaseF_ETH(uint _ETHFee) external override {
    ...
    if (totalYETIStaked != 0) {
      ETHFeePerYETIStaked = _ETHFee.mul(DECIMAL_PRECISION).div(totalYETIStaked);
    }
  ...
  function _getPendingYUSDGain(address _user) internal view returns (uint) {
    ...
    uint YUSDGain = stakes[_user].mul(F_YUSD.sub(F_YUSD_Snapshot)).div(
      DECIMAL_PRECISION
    );
    return YUSDGain;
  }
