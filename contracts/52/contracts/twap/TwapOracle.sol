...
contract TwapOracle is Ownable {
  ...
  function initialize(address _usdv, address _vader) external onlyOwner {
    require(VADER == address(0), "TwapOracle::initialize: Vader already set");
  ...
  function registerPair(
    ...
    require(token0 != token1, "TwapOracle::registerPair: Same token address");
    require(
    ...
    for (uint256 i = 0; i < pairCount; i++) {
      ...
      (
        ...
        uint32 blockTimestamp
          ...
          ? UniswapV2OracleLibrary.currentCumulativePrices(pairData.pair)
      ...
      unchecked {
        ...
        pairData.price0Average = FixedPoint.uq112x112(
          uint224(
            (price0Cumulative - pairData.price0CumulativeLast) / timeElapsed
          )
        );
        pairData.price1Average = FixedPoint.uq112x112(
          uint224(
            (price1Cumulative - pairData.price1CumulativeLast) / timeElapsed
          )
        );
      }
