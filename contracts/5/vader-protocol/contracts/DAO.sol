...
contract DAO {
  ...
  function _finalise(uint _proposalID) internal {
    ...
    emit ProposalFinalising(
      msg.sender,
      _proposalID,
      block.timestamp + coolOffPeriod,
      string(_type)
    );
  }
  ...
  function cancelProposal(uint oldProposalID, uint newProposalID) public {
    ...
    require(
      isEqual(
        bytes(mapPID_type[oldProposalID]),
        bytes(mapPID_type[newProposalID])
      ),
      "Must be same"
    );
  ...
  function finaliseProposal(uint proposalID) public {
    require(
      (block.timestamp - mapPID_timeStart[proposalID]) > coolOffPeriod,
      "Must be after cool off"
    );
  ...
  function grantFunds(uint _proposalID) internal {
    ...
    require(
      _grant.amount <= iERC20(USDV).balanceOf(VAULT) / 10,
      "Not more than 10%"
    );
  ...
  function countMemberVotes(
    uint _proposalID
  ) internal returns (uint voteWeight) {
