...
contract Vault {
  ...
  function _deposit(address _synth, address _member, uint _amount) internal {
    ...
    uint _weight = iUTILS(UTILS()).calcValueInBase(
      iSYNTH(_synth).TOKEN(),
      _amount
    );
  ...
  function harvest(address synth) external returns (uint reward) {
    ...
    if (iPOOLS(POOLS).isAsset(_token)) {
      ...
      _weight = iROUTER(ROUTER).getUSDVAmount(
        iUTILS(UTILS()).calcValueInBase(_token, reward)
      );
    }
  ...
  function calcCurrentReward(
    ...
    uint _secondsSinceClaim = block.timestamp -
      mapMemberSynth_lastTime[member][synth]; // Get time since last claim
  ...
  function calcReward(
    ...
    if (iPOOLS(POOLS).isAsset(iSYNTH(synth).TOKEN())) {
      uint _adjustedReserve = iROUTER(ROUTER).getUSDVAmount(reserveVADER()) +
        reserveUSDV(); // Aggregrate reserves
      return
        iUTILS(UTILS()).calcShare(
          _weight,
          totalWeight,
          _adjustedReserve / erasToEarn
  ...
  function withdraw(
    address synth,
    uint basisPoints
  ) external returns (uint redeemedAmount) {
