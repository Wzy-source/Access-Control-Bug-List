...
contract Pools {
  ...
  function addLiquidity(
    ...
    address token,
    address member
  ) external returns (uint liquidityUnits) {
  ...
  function removeLiquidity(
    ...
    address token,
    uint basisPoints
  ) external returns (uint outputBase, uint outputToken) {
  ...
  function _removeLiquidity(
    ...
    uint basisPoints,
    address member
  ) internal returns (uint outputBase, uint outputToken) {
    require(base == USDV || base == VADER);
    uint _units = iUTILS(UTILS()).calcPart(
      basisPoints,
      mapTokenMember_Units[token][member]
    );
    outputBase = iUTILS(UTILS()).calcShare(
      _units,
      mapToken_Units[token],
      mapToken_baseAmount[token]
    );
    outputToken = iUTILS(UTILS()).calcShare(
      _units,
      mapToken_Units[token],
      mapToken_tokenAmount[token]
    );
  ...
  function swap(
    ...
    if (toBase) {
      ...
      uint _swapFee = iUTILS(UTILS()).calcSwapFee(
        _actualInput,
        mapToken_tokenAmount[token],
        mapToken_baseAmount[token]
      );
  ...
  function mintSynth(
    ...
    uint _synthUnits = iUTILS(UTILS()).calcSynthUnits(
      _actualInputBase,
      mapToken_baseAmount[token],
      mapToken_Units[token]
    ); // Get Units
  ...
  // Safe adds
  function getAddedAmount(
    address _token,
    address _pool
  ) internal returns (uint addedAmount) {
