...
contract RCTreasury is Ownable, NativeMetaTransaction, IRCTreasury {
  ...
  function deposit(
    uint256 _amount,
    address _user
  ) public override balancedBooks returns (bool) {
  ...
  function withdrawDeposit(
    uint256 _amount,
    bool _localWithdrawal
  ) external override balancedBooks {
  ...
  function updateLastRentalTime(
    address _user
  ) external override onlyMarkets returns (bool) {
  ...
  /// @param _user the user address to query
  function userDeposit(address _user) external view override returns (uint256) {
  ...
  function rentOwedUser(
    address _user,
    uint256 _timeOfCollection
  ) internal view returns (uint256 rentDue) {
    return
  ...
  /// @param _user the user to query
  function depositAbleToWithdraw(
    address _user
  ...
  function foreclosureTimeUser(
    ...
    if (totalUserDailyRent > 0) {
      ...
      uint256 timeLeftOfDeposit = (depositAbleToWithdraw(_user) * 1 days) /
        totalUserDailyRent;
      ...
      if (foreclosureTimeWithoutNewCard > _timeOfNewBid) {
        ...
        uint256 _depositAtTimeOfNewBid = user[_user].deposit - _rentAlreadyOwed;
  ...
  function collectRentUser(
    address _user,
    uint256 _timeToCollectTo
  ) public override returns (uint256 newTimeLastCollectedOnForeclosure) {
    require(!globalPause, "Global pause is enabled");
