...
contract RCMarket is Initializable, NativeMetaTransaction, IRCMarket {
  ...
  function upgradeCard(uint256 _card) external onlyTokenOwner(_card) {
    ...
    require(
      !factory.trapIfUnapproved() || factory.isMarketApproved(address(this)),
      "Upgrade blocked"
    );
  ...
  function _payoutWinnings() internal {
    ...
    uint256 _remainingCut = ((((uint256(1000) - artistCut) - affiliateCut) -
  ...
  function _returnRent() internal {
    ...
    uint256 _rentCollectedAdjusted = (_rentCollected * _remainingCut) / (1000);
  ...
  function _processStakeholderPayment(
    uint256 _cut,
    address _recipient
  ) internal {
    if (_cut > 0) {
  ...
  function _collectRentAction(
    ...
    if (
      _user != address(this) && timeLastCollected[_card] < _timeOfThisCollection
    ) {
      ...
      if (_refundTime != 0) {
        uint256 _refundAmount = (_refundTime * cardPrice[_card]) / 1 days;
  ...
  function _collectRent(
    uint256 _card
  ) internal returns (bool didUpdateEverything) {
    uint32 counter = 0;
