...
contract RCOrderbook is Ownable, NativeMetaTransaction, IRCOrderbook {
  ...
  function addBidToOrderbook(
    ...
    if (_prevUserAddress == address(0)) {
      ...
      require(
        user[_prevUserAddress][index[_prevUserAddress][_market][_card]].price >=
          _price,
        "Location too low"
      );
    }
  ...
  function _searchOrderbook(
    ...
    while (
      ...
      _prevUser = _nextUser;
      _nextUser = user[_prevUser.next][index[_prevUser.next][_market][_card]];
  ...
  function _newBidInOrderbook(
    ...
    if (ownerOf[_market][_card] != _market) {
      (_prevUser, _price) = _searchOrderbook(_prevUser, _market, _card, _price);
    }
  ...
  function _updateBidInOrderbook(
    ...
    user[_currUser.next][index[_currUser.next][_market][_card]].prev = _currUser
      .prev;
    user[_currUser.prev][index[_currUser.prev][_market][_card]].next = _currUser
      .next;
    ...
    if (_owner && _currUser.prev == _market) {
      ...
      address _newOwner = user[_market][index[_market][_market][_card]].next;
      uint256 _newPrice = user[_newOwner][index[_newOwner][_market][_card]]
        .price;
      treasury.updateRentalRate(
        _user,
        _newOwner,
  ...
  function findNewOwner(
  ...
  ) external override onlyMarkets returns (address _newOwner) {
    address _market = msgSender();
    ...
    uint256 _oldPrice = user[_oldOwner][index[_oldOwner][_market][_card]].price;
  ...
  function removeUserFromOrderbook(
    address _user
  ) external override returns (bool _userForeclosed) {
    ...
    do {
      ...
      if (_tempPrev == user[_user][i].market) {
        ...
        uint256 _price = user[_tempNext][index[_tempNext][_market][_card]]
          .price;
        treasury.updateRentalRate(
          _user,
          _tempNext,
    ...
    while (
      ...
      for (uint256 i = market[_market].tokenCount; i != 0; ) {
        ...
        if (bidExists(_user, _market, i)) {
          // reduce bidRate
          uint256 _price = user[_user][index[_user][_market][i]].price;
          treasury.decreaseBidRate(_user, _price);

          // preserve linked list
          address _tempPrev = user[_user][index[_user][_market][i]].prev;
          address _tempNext = user[_user][index[_user][_market][i]].next;
  ...
  function bidExists(
    ...
    if (user[_user].length != 0) {
      ...
      if (index[_user][_market][_card] != 0) {
        ...
        if (user[_user][0].market == _market && user[_user][0].token == _card) {
          return true;
        }
      }
    }
  ...
  function getTimeHeldlimit(
    address _user,
    uint256 _card
  ) external view override onlyMarkets returns (uint256) {
  ...
  function setTimeHeldlimit(
    ...
    user[_user][index[_user][_market][_card]].timeHeldLimit = SafeCast.toUint64(
      _timeHeldLimit
    );
  }
