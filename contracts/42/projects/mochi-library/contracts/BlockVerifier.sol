...
library BlockVerifier {
    ...
    returns (bytes32 stateRoot, uint256 blockTimestamp, uint256 blockNumber)
  {
    assembly {
      ...
      let difficultyPointer, difficultyLength := readDynamic(
        difficultyPrefixPointer
      )
      let blockNumberPrefixPointer := add(difficultyPointer, difficultyLength)
      let blockNumberPointer, blockNumberLength := readDynamic(
        blockNumberPrefixPointer
      )
      ...
      let timestampPointer, timestampLength := readDynamic(
        timestampPrefixPointer
      )
