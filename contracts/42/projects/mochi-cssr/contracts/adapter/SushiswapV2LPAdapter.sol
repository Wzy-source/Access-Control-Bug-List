...
contract SushiswapV2LPAdapter is ICSSRAdapter {
    ...
    function support(address _asset) external view override returns (bool) {
        ...
        address calculatedAddress = SushiswapV2Library.pairFor(
            factory,
            underlying,
            weth
        );
    ...
    function update(
        address _asset,
        bytes memory _proof
    ) external override returns (float memory price) {
        address underlying = getUnderlyingAsset(IUniswapV2Pair(_asset));
    ...
    function getPrice(
        address _asset
    ) external view override returns (float memory price) {
    ...
    function _getPrice(
        IUniswapV2Pair _pair,
        address _underlying
    ) internal view returns (float memory price) {
        ...
        if (eCurr < eAvg) {
            ...
            uint256 sqrtd = ePool *
                ((ePool * 9) + ((aPool * 3988000 * eAvg) / Q112));
        ...
        return
            float({numerator: priceInEth, denominator: Q112}).mul(
